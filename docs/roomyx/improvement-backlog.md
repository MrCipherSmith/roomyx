...
| **R12** | The status bar drops the goal threshold. `GoalContract` carries `criteria` and `fail_below`/`pass_at_or_above`; `status-bar.ts` shows only `goal_statement`, truncated mid-word. The room exists to cross a number and the number is invisible. Should inherit R1's wrap primitive rather than hand-roll a second one. **Widened 2026-09-09:** the same single line also carries `setNotice`, so an owner command's *answer* is truncated too — a real veto against a dispatcher-less room replied `veto not accepted — No dispatcher is attached to this server, so there is nothing to act on the command. A`, stopping there. A refusal that cannot finish its own sentence is worse than a truncated goal, and it is the same fix. | 5-7 | half a day |
