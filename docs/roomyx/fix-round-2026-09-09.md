...
Claimed as published and not published — the first verifier caught that the
named evidence contained no table at all. Here it is. Casualties **and**
survivors, because a table with only casualties reads as cherry-picking. Each
mutation applied alone and reverted before the next.

Counts are of *this* table — 21 red, 7 green, 28 mutations — not of the earlier
prose, which said 27 and did not match anything. The second verifier caught that
too.
...
roster and in the chat view (1 each); the `isError` branch in `callTool` (2),
found by the first verifier contradicting the survivor list; and — after the
second verifier showed the subprocess test did not pin them — the server
shutdown's re-entry guard (1) and its `cleanup`-after-`close` ordering (1),
both now held by `test/server/shutdown-order.test.ts`.

**Green — the gate survives its own deletion:** `transport.onclose`, which is
unreachable from outside and labelled as such; `closeAllConnections()`; the
backoff ceiling; the roster `clip`'s `width <= 0` branch, unreachable as
constructed and labelled; and the three reconnect mechanisms, individually and
together.

Baseline before: 255 pass. After: 268 pass, 0 fail, tree clean.
...
- Dispositions in the block: **19 `acted-on`, 0 dismissed.** The nine originally
  filed as deferred were worked instead, because the completion gate refused to
  let the orchestrator dismiss on its own authority and the operator's answer
  was to fix them.
- Of the 19: 5 were regressions the fixes introduced, 4 were the verification
  record being wrong, 7 were the deferred set, and 3 needed no code change (two
  confirmations and this table).
- Corrected against a reviewer: 1 — `fix-03`'s memory impact — counted under
  `acted-on`, not separately.
- **Still open after all of it: 1.** `fix-11`, the reconnect coverage gap. The
  test written to close it does not, and says so in its own header.

This section has now been wrong twice and corrected twice. The first version
double-counted `fix-03`; the second was not updated when nine dispositions
flipped, while its own opening line claimed it had been corrected. Both were
caught by a verifier rather than by me.
...
[
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "commit c1297e3502555ef828ec7d69ceb8d321d79f0213 — the CLI re-entry guard and the cleanup ordering are now pinned by test/server/shutdown-order.test.ts, after a verifier showed the subprocess test did not pin either; transport.onclose and the clip width guard are labelled unreachable rather than claimed pinned"
  },
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "commit c1297e3502555ef828ec7d69ceb8d321d79f0213 — NOT closed: the test written for it does not reproduce the defect, and test/client/reconnect-rate.test.ts says so in its header. Recorded as the one gap this round leaves open."
  },
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "commit c1297e3502555ef828ec7d69ceb8d321d79f0213 — every condition-standing-in sleep in the two named files is a predicate wait through test/helpers/until.ts; the four setTimeout sites left are two hang detectors and two assertions about absence, where a duration is the right instrument"
  },
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "commit c1297e3502555ef828ec7d69ceb8d321d79f0213 — the table is published and its counts restated against itself (21 red, 7 green, 28 mutations); the stage counts restated against the dispositions the block actually carries"
  },
