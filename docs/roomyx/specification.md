  ...
  src/
    ...
    client/              # NEW (эта версия): TUI-клиент
      ...
      screens/
        ...
        help-overlay.ts      # полная карта клавиш по `?`
      components/
...
2. **Фильтр по участнику.** Открывается по `Enter`/клику на участнике в ростере, снимается по `Esc`. Лента фильтруется **на месте**, в той же панели, а не в модальном окне поверх неё — так фильтр складывается с прокруткой и поиском, и выход из него не отдельный режим, который надо помнить. Модалка (`agent-modal.ts`, до 0.5.0) удалена по итогам ревью TUI: её нельзя было прокрутить, она не сочеталась с поиском, стоила нажатия на выход и была прибита к `top: 2, left: 2`, из-за чего при ширине в 70% терминала ложилась поверх того самого ростера, из которого её открыли. Данные берутся из уже загруженного клиентом транскрипта; серверный `room.get_agent_detail` не изменён, клиенту он для этого больше не нужен. Явно НЕ показывается внутренняя трассировка рассуждений участника — это заявленный v1-предел из `prd.md`.
...
| R4 — просмотр одного агента | Data Contracts → `room.get_agent_detail`; TUI Client Architecture → фильтр по участнику | `implemented` — сервер отдаёт тул, UI фильтрует ленту на месте |
