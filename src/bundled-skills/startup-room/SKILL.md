---
name: startup-room
description: Runs a multi-persona room: several named subagents hold one evolving discussion toward an explicit, measurable goal. Each persona is spawned once and kept alive, so every turn sends only what that participant has not seen rather than the whole transcript; one dispatcher is the log's sole writer, and every message is broadcast to everyone, not only to whoever was @-addressed. The room is goal-driven, not round-driven: it runs until the goal is met and the participants agree, or until the owner stops it. Use when the user wants founders, experts, interview personas or judges to debate, brainstorm, interview each other or converge on a decision over several rounds — especially when they ask for that without re-pasting the conversation into a fresh agent each round.
---
