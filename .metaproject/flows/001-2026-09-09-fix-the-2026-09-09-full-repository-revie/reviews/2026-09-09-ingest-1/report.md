...
- **Seven gates survive their own deletion** with all tests green:
  `transport.onclose`, `closeAllConnections()`, `runUntilSignal`'s re-entry
  guard, `cleanup()` inside `.finally()`, the backoff ceiling, the `isError`
  branch (subsumed by the JSON-parse fallback throwing the same `ToolError`),
  and the roster `clip`'s `width <= 0` branch (unreachable as constructed).
...
- Dispatched: 3 reviewers (logic, testing-practices, security-code).
- Raised: 19 findings.
- Reproduced independently by the orchestrator before acting: 6.
- Fixed in this round: 9 (5 regressions, 4 verification/coverage).
- Corrected against the reviewer: 1.
- Left open with a reason: 9.
- Refuted outright: 0.
...
[
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "7dde8b2; severity corrected downward, impact refuted by measurement"
  },
  "class_scope": {
   "sites": [
 ...
 },
 {
  ...
  "disposition": {
   ...
   "evidence": "docs/roomyx/fix-round-2026-09-09.md, Open section"
  }
 },
 {
  "id": "fix-11",
 ...
 },
 {
  ...
  "disposition": {
   ...
   "evidence": "outside the fix diff; recorded in docs/roomyx/fix-round-2026-09-09.md"
  }
 },
 {
  "id": "fix-16",
  ...
  "disposition": {
   "state": "dismissed-out-of-scope",
   "evidence": "pre-existing behaviour, recorded"
  }
 },
 {
  "id": "fix-17",
 ...
 },
 {
  ...
  "disposition": {
   "state": "acted-on",
   "evidence": "docs/roomyx/fix-round-2026-09-09.md"
  }
 },
 {
  "id": "fix-19",
